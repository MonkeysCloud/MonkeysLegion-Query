...
final class QueryBuilder
{
    ...
    private function findAliasForColumn(string $column): ?array
    {
        $candidates = [];
    ...
    private function snakeToCamelId(string $col): string
    {
    ...
    private function camelToSnakeId(string $col): string
    {
        if (!str_ends_with($col, '_id')) return $col;
        $base = substr($col, 0, -3);
    ...
    private function resolveColumnForAlias(string $alias, string $column): ?string
    {
        $alias = trim($alias, ' `');
        $column = trim($column, ' `');
    ...
    private function normalizeColumnsClause(string $clause, bool $qualifyBare = true): string
    {
